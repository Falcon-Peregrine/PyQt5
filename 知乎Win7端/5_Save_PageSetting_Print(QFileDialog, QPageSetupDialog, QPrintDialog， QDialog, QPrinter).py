...
class Example(QWidget):
	...
	def __init__(self):
		super().__init__()
		self.printer = QPrinter() # 因为下面代码中QPageSetupDialog、QPrintDialog涉及到QPrinter()对象，所以将其在类初始化的时候生成，便于函数的调用。
		self.initUI()
	...
	def initUI(self):
		...
		self.setWindowTitle('保存、打印文件, 打开多个文件')
	...
	def openfiles(self):
		fnames = QFileDialog.getOpenFileNames(self, 'Open Multi Files', './')
		if fnames[0]:
