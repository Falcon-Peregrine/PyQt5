...
class PasswdDialog(QDialog):
    ...
    def eventFilter(self, object, event): # 重实现了QDialog的eventFilter方法， 由self.edit.installEventFilter(self)管理， True的话事件被过滤， False就没事
        if object == self.edit:
    ...
    def Ok(self):
        ...
        if len(self.text) == 0:
            ...
            self.done(1)          #  使用done语句关闭对话框并将其结果设置为一个整数
